...
async fn guest_sees_register() {
    ...
    assert!(
        app.menu.iter().any(|e| e.action == MenuAction::Builtin(MenuItem::Register)),
...
async fn registered_user_does_not_see_register() {
    ...
    assert!(app.menu.iter().any(|e| e.action == MenuAction::Builtin(MenuItem::Boards)));
    assert!(app.menu.iter().any(|e| e.action == MenuAction::Builtin(MenuItem::Mail)));
    assert!(app.menu.iter().any(|e| e.action == MenuAction::Builtin(MenuItem::Quit)));
}
...
async fn oneliners_menu_follows_feature_toggle() {
    ...
    assert!(app.menu.iter().any(|e| e.action == MenuAction::Builtin(MenuItem::Oneliners)));
