...
async fn door_launches_on_a_pty_over_websocket() {
    ...
    // Drive the menu: 8× Down reaches "Door Games" (Bulletins, Boards,
    // Oneliners, Polls, Mail, Who, Stats, Search, → Doors), Enter opens the door
    // list, Enter launches the first door. Keys are sent as raw terminal bytes
    // (the transport decodes them exactly like a real client).
