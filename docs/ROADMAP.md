...
| [**File areas**](https://github.com/AdamIsrael/bbs-rs/issues/6) | L | Upload/download file areas with descriptions and quotas — a cornerstone of classic BBSes. | **Phase 1 shipped**: `file_areas`/`files` schema, in-BBS browser, `[files]` quota + extension limits, FS storage, `bbsctl` area/file management. Live SFTP transfer tracked in [#38](https://github.com/AdamIsrael/bbs-rs/issues/38). |
